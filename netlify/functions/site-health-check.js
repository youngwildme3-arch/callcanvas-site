...
// Runs daily at 6:10 AM Central (11:10 UTC), 10 minutes after the daily blog publishes.
// Tests all critical endpoints and pages.
// If site itself is down (homepage/blog 5xx): triggers Netlify rebuild automatically.
// If backend functions fail: logs and writes status, but does NOT rebuild
// (those are usually transient API issues that fix themselves).
// Always writes public/_health.json so latest status is viewable at /_health.json
...
const TESTS = [
  ...
  { name: 'chat_function', method: 'POST', path: '/.netlify/functions/chat', body: { messages: [{ role: 'user', content: 'health check ping' }] }, expect_status: [200, 500], critical: false },
...
async function runTest(test) {
  ...
  try {
    ...
    if (test.expect_text) {
      ...
      if (!content_ok) content_msg = 'expected text "' + test.expect_text + '" not found';
    }
    if (test.expect_json_keys) {
      try {
        ...
        content_ok = false;
        content_msg = 'invalid JSON';
      }
    }

    const ok = status_ok && content_ok;
    return {
      name: test.name,
  ...
  try {
    ...
    const body = {
      message: status.overall_ok
        ...
        : 'Health check FAILED ' + status.timestamp + ' (' + status.failed_count + ' tests)',
      content
    };
    if (sha) body.sha = sha;
...
exports.handler = async (event) => {
  ...
  // Always write status file (lets you check /_health.json anytime)
